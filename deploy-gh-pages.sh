...
# Finance Management Web - GitHub Pages 部署脚本
...
REPO_NAME=$(git remote get-url origin 2>/dev/null | sed 's/.*\///;s/\.git$//' || echo "Finance-Management-Web")
echo "📦 仓库名: $REPO_NAME"
...
# 构建 Vite 项目到外部目录
echo "📦 构建 Vite 项目到外部目录..."
BUILD_DIR="../finance_management_web_build"
...
# 设置 GitHub Pages 的 base 路径（如果不是根域名仓库）
# 如果仓库名是 username.github.io，则不需要 base 路径
if [[ "$REPO_NAME" == *.github.io ]]; then
    BASE_PATH=""
    echo "🔧 检测到根域名仓库，不使用 base 路径"
else
    BASE_PATH="/$REPO_NAME"
    echo "🔧 设置 base 路径: $BASE_PATH"
fi
