...
export default function Settings() {
    ...
    const handleExportBackup = () => {
        ...
        if (!dbData) {
            setBackupStatus(t('settings.noDataToBackup'));
            return;
        }
        ...
        setBackupStatus(t('settings.backupExported'));
    ...
    const handleImportBackup = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (!backup.data) {
                    setBackupStatus(t('settings.invalidBackup'));
                    return;
                }
                ...
                setBackupStatus(t('settings.backupLoaded'));
            } catch {
                setBackupStatus(t('settings.failedToParse'));
            }
        };
    ...
    const handleRestoreBackup = () => {
        ...
        setBackupStatus(t('settings.dataRestored'));
        setTimeout(() => {
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 dark:border-gray-700/30 p-6 hover:shadow-2xl hover:-translate-y-1 transition-all duration-500">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                        <span className="text-2xl">🌐</span> {t('settings.language')}
                    </h3>
                ...
                <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 dark:border-gray-700/30 p-6 hover:shadow-2xl hover:-translate-y-1 transition-all duration-500">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                        <span className="text-2xl">💱</span> {t('settings.currency')}
                    </h3>
                    <div className="space-y-2">
                        {['¥', '$', '€', '£'].map(curr => (
                            <button
                ...
                <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 dark:border-gray-700/30 p-6 hover:shadow-2xl hover:-translate-y-1 transition-all duration-500">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                        <span className="text-2xl">📅</span> {t('settings.startOfWeek')}
                    </h3>
                    <div className="space-y-2">
                        {(['sunday', 'monday'] as const).map(day => (
                            ...
                            >
                                {day === 'sunday' ? t('settings.sunday') : t('settings.monday')}
                            </button>
                        ))}
                    </div>
                </div>
                ...
                <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 dark:border-gray-700/30 p-6 hover:shadow-2xl hover:-translate-y-1 transition-all duration-500">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                        <span className="text-2xl">✨</span> {t('settings.display')}
                    </h3>
                    <div className="space-y-4">
                        <label className="flex items-center justify-between cursor-pointer">
                            <span className="text-gray-700 dark:text-gray-300">{t('settings.compactMode')}</span>
                            <button
                        ...
                        <label className="flex items-center justify-between cursor-pointer">
                            <span className="text-gray-700 dark:text-gray-300">{t('settings.showPet')}</span>
                            <button
                        ...
                        <label className="flex items-center justify-between cursor-pointer">
                            <span className="text-gray-700 dark:text-gray-300">{t('settings.showParticles')}</span>
                            <button
                ...
                <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 dark:border-gray-700/30 p-6 hover:shadow-2xl hover:-translate-y-1 transition-all duration-500">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                        <span className="text-2xl">💾</span> {t('settings.backup')}
                    </h3>
                ...
                <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 dark:border-gray-700/30 p-6 hover:shadow-2xl hover:-translate-y-1 transition-all duration-500">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                        <span className="text-2xl">📊</span> {t('settings.dataStats')}
                    </h3>
                    <div className="space-y-2 text-sm">
                        <div className="flex justify-between text-gray-700 dark:text-gray-300">
                            <span>{t('settings.transactions')}</span>
            ...
            <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-xl border border-red-200 dark:border-red-800/30 p-6">
                <h3 className="text-lg font-semibold text-red-600 dark:text-red-400 mb-4 flex items-center gap-2">
                    <span className="text-2xl">⚠️</span> {t('settings.dangerZone')}
                </h3>
                <button
